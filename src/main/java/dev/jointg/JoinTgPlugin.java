...
public final class JoinTgPlugin extends JavaPlugin {
  ...
  public void onEnable() {
    ...
    String quitMessage = getConfig().getString("quit-message", "Player %player% left");
    String deathMessage = getConfig().getString("death-message",
        "Player %player% died because of %reason% at %world% (%x%, %y%, %z%)");
    String startMessage = getConfig().getString("start-message", "Server started!");

    if (botToken.isEmpty() || chatId.isEmpty()) {
      getComponentLogger().warn("Telegram bot token or chat ID is missing; Telegram notifications disabled.");
      notifier = TelegramNotifier.disabled(getComponentLogger());
    } else {
      notifier = TelegramNotifier.enabled(getComponentLogger(), botToken, chatId, joinMessage, quitMessage,
          deathMessage, startMessage);
    }

    String title = getConfig().getString("join-title", "<gradient:green:blue>Welcome <player>!");
    String subtitle = getConfig().getString("join-subtitle", "<gray>Enjoy your stay");
    ...
    getServer().getPluginManager()
        .registerEvents(new PluginListener(this, notifier, title, subtitle, fadeIn, stay, fadeOut), this);
